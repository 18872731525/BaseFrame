...
public class PageScrollView extends ViewGroup {
    ...
    public boolean shouldTakeInterraper = false;
    /**
     * 用于滚动的实例对象
     */
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (diff > mTouchSlop && shouldTakeInterraper) {
                    return true;
                }
                break;
        }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (direction == 1) {
                    ...
                    Logger.d("GG:getScrollY():" + getScrollY());
                    Logger.d("GG:getHeight():" + getHeight());
                    float tag = getHeight() / 5.0f;

                    int dy = getScrollY() - currentPosition * getHeight();
                    if (dy > 0) {
                        if (dy >= tag) {
                            currentPosition++;
                            mScroller.startScroll(0, getScrollY(), 0, getHeight() * (currentPosition) - getScrollY());
                        } else {
...
//                        if (-dy >= tag) {
//                            //翻页
//                            mScroller.startScroll(0, getScrollY(), 0, getHeight() * (currentPosition + 1));
//                        } else {
//                            mScroller.startScroll(0, getScrollY(), 0, dy);
//                        }
//                    } else {
//                        if (dy >= tag) {
//                            mScroller.startScroll(0, getScrollY(), 0, getHeight() * ((currentPosition - 1) < 0 ? 0 : currentPosition - 1));
//                        }else{
//                            mScroller.startScroll(0, getScrollY(), 0, dy);
//                        }
//                    }
...
//                    mScroller.startScroll(0, getScrollY(), 0, dy);
    ...
    public interface BackTopLister {
        void whenBackTop();
    }

    public void setLister(BackTopLister lister) {
        backTopLister=lister;
    }
